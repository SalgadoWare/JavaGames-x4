...
public class ConcurrentDeepeningMinMax {
	...
	private static final Logger log = Logger.getLogger(ConcurrentDeepeningMinMax.class.getName());

	private MinMax.Node<?, ?> bestNode;
	private int threads;

	/**
	 * Counts the number of board evaluations within min-max, across all
	 * threads. Since this is updated by concurrent threads, we use
	 * AtomicInteger to guarantee that no data races will occur.
	 */
	 ...
	 * 
	 * @param threads
	 *            to run concurrently. The actual number of concurrently running
	 *            threads can be calculated as Min(threads, actualProcessors,
	 *            validMovesInInitialState)
	 */
	...
	/**
	 * Searches for the best action for the specified player in the provided
	 * state. Search-depth is increased until the time runs out, and the deepest
	 ...
	 *            that is due to move
	 * @param millis
	 *            (ms) to invest in searching this space
	 * @param <S>
	 *            state type
	 * @param <A>
	 *            action (move) type
	 * @return a node with the best move and its expected value. If no level was
	 *         fully explored, returns a random move with -Infinity as value.
	 *         Once this method has executed, getEvaluationCount returns the
	 *         total number of board evaluations performed during the search.
	 */
	@SuppressWarnings("unchecked")
	public <S extends GameState<S, A>, A extends GameAction<S, A>> MinMax.Node<S, A> chooseNode(S state,
			int playerNumber, int millis) {
		...
		bestNode = new MinMax.Node<>(validActions.get(new Random().nextInt(validActions.size())),
			...
			try {
				...
				for (A a : validActions) {
					level.add(executor.submit(() -> {
						// calls min-max on the state that results after
						// applying a
						S afterMove = a.applyTo(state);
						return new MinMax.Node<>(a, -mm.chooseNode(afterMove.getTurn(), afterMove).getValue());
					}));
				}
				...
				// and send InterruptedException to those that did not finish in
				// time
				...
				for (Future<MinMax.Node<S, A>> f : level) {
					if (f.get().getValue() > bestNode.getValue()) {
						// if a Future was interrupted, get throws
						// ExecutionException
